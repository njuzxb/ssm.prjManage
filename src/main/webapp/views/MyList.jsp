		...
		<div class="row">
			...
			<div class="col-md-6">
				<ul class="nav nav-pills nav-justified">
				  <li role="presentation" class="active"><a href="${APP_PATH}/My/MyList">我的项目</a></li>
	...
	<script type="text/javascript">
		...
		function build_emps_table(result){
			...
			$.each(prjs,function(index,item){
				...
				var processTd = $("<td></td>").append(item.process+"%");
		...
		$("#prj_update_btn").click(function(){
			...
			//发送ajax请求保存更新的员工数据
			 $.ajax({
				url:"${APP_PATH}/myprj/"+$(this).attr("edit-id"),
				type:"PUT",
				data:$("#prjUpdateModal form").serialize()+"&content="+encodeURIComponent(editorUpdate.txt.html()),
				success:function(result){
					if(result.code == 100){
						//alert(result.msg);
						//1、关闭模态框
		...
		//点击提交，把项目提交审核
		 $(document).on("click",".finish_btn",function(){
			...
			//alert($(this).parents("tr").find("td:eq(0)").text());
			 if(confirm("确认把【"+prjName+"】项目提交审核？")){
				//确认，发送ajax请求修改
				$.ajax({
					url:"${APP_PATH}/myprj/"+$(this).parents("tr").find("td:eq(0)").text(),
					type:"PUT",
					data:"status=待审核",
					success:function(result){
						if(result.code == 100){
