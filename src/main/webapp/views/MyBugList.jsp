        ...
        <form class="form-horizontal">
		  ...
		  <div class="form-group">
		   ...
		   <div class="form-group">
		    <label class="col-sm-2 control-label">ModuleName</label>
		    <div class="col-sm-5">
		   	 <input type="text" name="moduleid" class="form-control" id="mod_update_input" readonly>
		    </div>
		  </div>
		  <div class="form-group">
		    <label class="col-sm-2 control-label">项目描述</label>
		...
		<jsp:include page="/Navigation.jsp"></jsp:include>
			<div class="col-md-6">
				<ul class="nav nav-pills nav-justified">
				  ...
				  <li role="presentation" class="active"><a href="${APP_PATH}/My/MyBugList">我的BUG</a></li>
				</ul>
			</div>
		</div>
		<!-- 按钮 -->
	...
	<script type="text/javascript">
		 ...
		 function getBug(id){
			$.ajax({
				...
				success:function(result){
					...
					$("#prj_update_input").val(bugData.project.projectname);
					$("#mod_update_input").val(bugData.module==null?null:bugData.module.modulename);
		...
		//点击更新，更新BUG信息
 		$("#bug_update_btn").click(function(){
			...
			//发送ajax请求保存更新的员工数据
			 $.ajax({
				url:"${APP_PATH}/mybug/"+$(this).attr("edit-id"),
				type:"PUT",
		...
		//点击提交
 		 $(document).on("click",".finish_btn",function(){
			...
			//alert($(this).parents("tr").find("td:eq(0)").text());
			 if(confirm("确认把【"+bugName+"】BUG提交审核？")){
				//确认，发送ajax请求修改
				$.ajax({
					url:"${APP_PATH}/mybug/"+$(this).parents("tr").find("td:eq(0)").text(),
					type:"PUT",
					data:"status=待审核",
					success:function(result){
						if(result.code == 100){
